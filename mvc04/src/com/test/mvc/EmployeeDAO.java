...
public class EmployeeDAO implements IEmployeeDAO
{
	...
	public int getMinBasicPay(String positionId) throws SQLException
	{
		...
		PreparedStatement pstmt = conn.prepareStatement(sql);
		pstmt.setInt(1, Integer.parseInt(positionId));	// check~!!!
		// ※ 주의
		// 여기서 setString 을 하느냐, setInt 하느냐의 결정은
		// ? 에 뭘 넣어주느냐 → Oracle 에 전달
		// Oracle 상에서는 POSITIONID 가 NUMBER로 되어있기 때문에
		// 정수형태로 형변환 → setInt 로 값을 넘겨야 함
		ResultSet rs = pstmt.executeQuery();
	...
	public int remove(String employeeId) throws SQLException
	{
		...
		PreparedStatement pstmt = conn.prepareStatement(sql);
		pstmt.setInt(1, Integer.parseInt(employeeId));	// check~!!!
	...
	// 직원 검색
	// 수정이나 삭제를 위해 검색하는건 UPDATE 쿼리문에 준하는 데이터를
	// 얻어오면 된다.
	// INSERT 폼에 끼워넣어 수정폼을 채우는 형식으로 사용될 것.
	@Override
	public Employee searchId(String employeeId) throws SQLException
	{
		...
		while (rs.next())
		{
			...
			employee.setTelephone(rs.getString("TELEPHONE"));
			employee.setDepartmentId(rs.getString("DEPARTMENTID"));
			employee.setPositionId(rs.getString("POSITIONID"));	// 이땐 String 으로
			employee.setRegionId(rs.getString("REGIONID"));
			employee.setBasicPay(rs.getInt("BASICPAY"));
			employee.setExtraPay(rs.getInt("EXTRAPAY"));
			//employee.setPay(rs.getInt("PAY"));
