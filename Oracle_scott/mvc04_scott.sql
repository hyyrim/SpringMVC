...
-- 잘못 작성해서 ROLLBACK / 테이블 삭제 / 시퀀스 삭제
ROLLBACK;
...
INSERT INTO DEPARTMENT(DEPARTMENTID, DEPARTMENTNAME)
VALUES(DEPARTMENTSEQ.NEXTVAL, '마케팅 부');
INSERT INTO DEPARTMENT(DEPARTMENTID, DEPARTMENTNAME)
...
1	           한혜림	"?h????"	    1997-10-06	0	    010-8755-9495
...
--○ 부서 데이터 조회 쿼리문 구성(직위 데이터 삭제 가능여부 확인)
...
1	        한혜림	1997-10-06	0	    010-8755-9495	?
...
EMPLOYEEID	NAME	BIRTHDAY	LUNAR	TELEPHONE	
1	        한혜림	1997-10-06	0	    010-8755-9495	
2	        박정준	1996-10-31	0	    010-4020-7429	
...
SELECT * FROM EMPLOYEEVIEW WHERE EMPLOYEEID=1
;
...
--○ 뷰(REGIONVIEW) 조회를 통한 부서 리스트 출력 쿼리문 구성
SELECT REGIONID, REGIONNAME, DELCHECK
FROM REGIONVIEW;


--○ 뷰(POSITIONVIEW) 조회를 통한 부서 리스트 출력 쿼리문 구성
...
SELECT EMPLOYEEID, NAME, SSN1, TO_CHAR(BIRTHDAY, 'YYYY-MM-DD') AS BIRTHDAY
